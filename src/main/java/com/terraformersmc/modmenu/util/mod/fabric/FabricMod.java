...
//		}
	...
	public @NotNull DynamicTexture getIcon(FabricIconHandler iconHandler, int i) {
		...
		if (icon == null) {
			...
			return iconHandler.createIcon(FabricLoader.getInstance().getModContainer(ModMenu.MOD_ID).orElseThrow(() -> new RuntimeException("Cannot get ModContainer for Fish mod with id " + ModMenu.MOD_ID)), "assets/" + ModMenu.MOD_ID + "/unknown_icon.png");
		}
		return icon;
	}
	...
	public @NotNull String getTranslatedDescription() {
		...
		if (getId().equals("java")) {
			description = description + "\n" + I18n.getStringParams("modmenu.javaDistributionName", getName());
		}
		return description;
	}
