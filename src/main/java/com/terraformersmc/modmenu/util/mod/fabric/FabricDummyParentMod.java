...
public class FabricDummyParentMod implements Mod {
	...
	public @NotNull DynamicTexture getIcon(FabricIconHandler iconHandler, int i) {
		...
		ModContainer iconSource = FabricLoader.getInstance().getModContainer(iconSourceId).orElseThrow(() -> new RuntimeException("Cannot get ModContainer for Fish mod with id " + finalIconSourceId));
