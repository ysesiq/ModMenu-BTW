...
public class ParentEntry extends ModListEntry {
	...
	public void render(int index, int x, int y, int rowWidth, int rowHeight, BufferBuilder bufferBuilder, int mouseX, int mouseY, boolean hovered) {
		...
		ChatMessageComponent str = shownChildren == children.size() ? ChatMessageComponent.createFromText(String.valueOf(shownChildren)) : ChatMessageComponent.createFromText(shownChildren + "/" + children.size());
		int childrenWidth = font.getStringWidth(str.toStringWithFormatting(true)) - 1;
		...
		font.drawString(str.toStringWithFormatting(true), (int) (childrenBadgeX + (float) childrenBadgeWidth / 2 - (float) childrenWidth / 2), childrenBadgeY + 1, 0xCACACA);
